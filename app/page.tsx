"use client";

import { useSession, signIn, signOut } from "next-auth/react";
import Image from "next/image";
import { useState } from "react";
import { useTeamName } from "@/components/useTeamName";
import TeamNameSetup from "@/components/TeamNameSetup";
import Transfers from "@/components/Transfers";

const TABS = ["My Team", "Points", "Transfers", "League"] as const;
type Tab = (typeof TABS)[number];

export default function Home() {
  const { data: session, status } = useSession();
  const [activeTab, setActiveTab] = useState<Tab>("My Team");
  const { teamName, saveTeamName } = useTeamName(session?.user?.email);

  if (status === "loading") {
    return <main className="min-h-screen bg-white" />;
  }

  if (session) {
    if (teamName === null) {
      return <main className="min-h-screen bg-white" />;
    }

    if (teamName === "") {
      return <TeamNameSetup onSubmit={saveTeamName} />;
    }

    return (
      <div className="min-h-screen bg-white flex flex-col">
        <nav className="border-b border-gray-200 px-6 flex items-center h-16 gap-8">
          {/* Logo */}
          <div className="w-10 h-10 relative flex-shrink-0">
            <Image
              src="/fc-mollan-logo.svg"
              alt="FC Möllan"
              fill
              className="object-contain"
              priority
            />
          </div>

          {/* Tabs */}
          <div className="flex items-center gap-1">
            {TABS.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${
                  activeTab === tab
                    ? "bg-gray-100 text-gray-900"
                    : "text-gray-500 hover:text-gray-900 hover:bg-gray-50"
                }`}
              >
                {tab}
              </button>
            ))}
          </div>

          {/* Sign out pushed to the right */}
          <button
            onClick={() => signOut()}
            className="ml-auto text-sm text-gray-400 hover:text-gray-600 transition-colors"
          >
            Sign out
          </button>
        </nav>

        <main className="flex-1 px-6 py-8">
          {activeTab === "My Team" && (
            <h2 className="text-2xl font-semibold text-gray-900">{teamName}</h2>
          )}
          {activeTab === "Transfers" && <Transfers />}
        </main>
      </div>
    );
  }

  return (
    <main className="min-h-screen bg-white flex items-center justify-center">
      <button
        onClick={() => signIn("google")}
        className="flex items-center gap-3 px-6 py-3 border border-gray-300 rounded-full shadow-sm hover:shadow-md transition-shadow bg-white text-gray-700 font-medium"
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
          />
        </svg>
        Sign in with Google
      </button>
    </main>
  );
}
